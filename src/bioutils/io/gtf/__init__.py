...
from bioutils.io.gtf.gtf_record import GtfRecord
...
class GtfIterator(_GtfView):
    ...
    def __iter__(self) -> Iterator[GtfRecord]:
        ...
        with ioctl.get_reader(realname) as reader:  # 268435456
            all_record = reader.readlines()

        for line in tqdm(iterable=all_record, desc=f"Reading {realname}", total=ioctl.wc_l(realname)):
            line = line.strip()
