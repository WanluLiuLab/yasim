...
#
#  NAME: __main__.py -- bioutils main frontend.
#
#  VERSION HISTORY:
...
# ==============================================================================
"""
__main__.py -- bioutils main frontend.
...
1) Search for all valid sub-command in :py:mod:`bioutils.main`;
...
A valid subcommand should be a module under :py:mod:`bioutils.main` with :py:func:`main` function defined.
...
for spec in pkgutil.iter_modules(
        bioutils.main.__dict__["__spec__"].submodule_search_locations):
...
def _get_main_func_from_subcommand(name: str):
    ...
    if name in valid_subcommand_names:
        __import__(f'bioutils.main.{name}')
        i = bioutils.main.__dict__[name]
...
def main():
    ...
    Should be invoked by :py:mod:`bioutils` only.
    """
