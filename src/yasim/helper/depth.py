...
def simulate_gene_level_depth_gmm(gv: GeneTree, mu: float, low_cutoff: float, high_cutoff_ratio: float) -> DepthType:
    """
    ...
    for i, gene_id in enumerate(gv.gene_ids):
        depth[gene_id] = data[i]
    return depth


def simulate_isoform_variance_inside_a_gene(
    n: int, mu: float, alpha: int, low_cutoff: float, high_cutoff_ratio: float
) -> List[float]:
    """
