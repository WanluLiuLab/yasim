...
def create_parser() -> argparse.ArgumentParser:
    parser = ArgumentParserWithEnhancedFormatHelp(
        prog="python -m yasim pbsim2",
        description=__doc__.splitlines()[1],
    )
    parser = llrg.patch_frontend_parser_public(parser, llrg_name="pbsim2", default_llrg_executable_name="pbsim2")
...
def main(args: List[str]):
    ...
    return bulk_rna_seq_frontend(
        ...
        jobs=args.jobs,
        simulator_name=(
            "_".join(("pbsim2", args.hmm_model, "clr")) if args.simulator_name is None else args.simulator_name
        ),
        adapter_args={
