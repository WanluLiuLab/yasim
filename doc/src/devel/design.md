...
RPKM
: Read Per Killo base per Million mapped reads
...
GEP
: A profile with Transcript ID - Absolute Abundance of that Transcript ($t_{ID} \rightarrow (Y_{t})$).
...
From literature of MicroArray [^Furusawa2003] [^Ueda2004] [^Lu2005] [^Brown2007], we can infer that the distribution of GEP of general organisms obeys Zipf's Law [^Zipf1949] and can be fitted into a Log-Normial Distribution. This opnion was challanged by [^Nowrousian2013].
...
1. The GEP accross transcripts in one sample can be fitted into a log-normal (LNorm) distribution.
...
2. The GEP accross transcripts in one sample obeys Ziff's Law.
...
3. Given the relative abundance of a transcript, its abundance across samples is negative binomial (NB) distributed.
